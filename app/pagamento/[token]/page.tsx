...
export default async function PaymentPage({params}:{params:Promise<{token:string}>}){
  ...
  if(!quote||!["quote_sent","payment_reported"].includes(quote.status))return <PaymentUnavailable/>;
  ...
  return <main className="payment-page"><header className="payment-header"><a href="/"><img src="/logo-valf-suite.png" alt="VALF Suite"/></a></header><section className="payment-intro"><p className="eyebrow">VALF Suite · Prenotazione</p><h1>Comunica il pagamento</h1><p>Compila il modulo dopo aver effettuato l’acconto. La prenotazione sarà confermata soltanto dopo la verifica dell’accredito.</p></section><PaymentForm token={token} language={quote.language} guestName={quote.name} arrival={date(quote.arrivalDate)} departure={date(quote.departureDate)} guests={quote.guestCount} totalCents={quote.amountCents} today={todayAtProperty()}/><footer className="payment-footer">VALF Suite · Arcola, Liguria</footer></main>;
}
