...
export async function POST(request: Request) {
  ...
  const data = await request.json().catch(() => null) as { id?: unknown; subject?: unknown; body?: unknown; price?: unknown; requestedPayment?:unknown; depositPercent?:unknown; balancePercent?:unknown } | null;
  if (!data || typeof data.id !== "string" || typeof data.subject !== "string" || typeof data.body !== "string" || typeof data.price !== "string" || typeof data.depositPercent !== "number" || typeof data.balancePercent !== "number") return Response.json({ message: "Preventivo non valido." }, { status: 400 });
  const subject = data.subject.trim();
  const body = data.body.trim();
  const price = data.price.trim().replace(",", ".");
  if (!subject || subject.length > 180 || !body || body.length > 6000 || !/^\d+(\.\d{1,2})?$/.test(price) || Number(price) <= 0 || !Number.isInteger(data.depositPercent) || !Number.isInteger(data.balancePercent) || data.depositPercent < 0 || data.depositPercent > 100 || data.balancePercent < 0 || data.balancePercent > 100 || data.depositPercent + data.balancePercent !== 100) return Response.json({ message: "Controlla oggetto, testo e importo del preventivo." }, { status: 400 });
  ...
  const balancePercent=data.balancePercent;
  const requestedPaymentCents = Math.round(quoteAmountCents * depositPercent / 100);
  ...
  const paymentConditions:Record<string,string>=requestedPaymentCents===quoteAmountCents?{it:`Per confermare la prenotazione è richiesto il pagamento dell’intero importo, pari a ${currency.format(requestedPaymentCents/100)}.`,en:`To confirm the booking, full payment of ${currency.format(requestedPaymentCents/100)} is required.`,fr:`Pour confirmer la réservation, le paiement intégral de ${currency.format(requestedPaymentCents/100)} est demandé.`,es:`Para confirmar la reserva, se requiere el pago completo de ${currency.format(requestedPaymentCents/100)}.`,de:`Zur Bestätigung der Buchung ist die vollständige Zahlung von ${currency.format(requestedPaymentCents/100)} erforderlich.`}:{it:`Per confermare la prenotazione è richiesto un primo pagamento del ${depositPercent}%, pari a ${currency.format(requestedPaymentCents/100)}. Il saldo residuo del ${balancePercent}%, pari a ${currency.format(balanceAmountCents/100)} dovrà essere versato entro il ${localizedBalanceDueDate}.`,en:`To confirm the booking, an initial payment of ${depositPercent}% (${currency.format(requestedPaymentCents/100)}) is required. The remaining ${balancePercent}% balance of ${currency.format(balanceAmountCents/100)} must be paid by ${localizedBalanceDueDate}.`,fr:`Pour confirmer la réservation, un premier paiement de ${depositPercent} %, soit ${currency.format(requestedPaymentCents/100)}, est demandé. Le solde de ${balancePercent} %, soit ${currency.format(balanceAmountCents/100)} devra être réglé au plus tard le ${localizedBalanceDueDate}.`,es:`Para confirmar la reserva, se requiere un primer pago del ${depositPercent}%, equivalente a ${currency.format(requestedPaymentCents/100)}. El saldo restante del ${balancePercent}%, equivalente a ${currency.format(balanceAmountCents/100)} deberá abonarse antes del ${localizedBalanceDueDate}.`,de:`Zur Bestätigung der Buchung ist eine erste Zahlung von ${depositPercent}% (${currency.format(requestedPaymentCents/100)}) erforderlich. Der Restbetrag von ${balancePercent}% (${currency.format(balanceAmountCents/100)}) ist bis zum ${localizedBalanceDueDate} zu zahlen.`};
  const replacements:Record<string,string> = {
    "{PREZZO}": currency.format(quoteAmountCents / 100),
    "{ACCONTO}": currency.format(requestedPaymentCents / 100),
    "{PERCENTUALE_ACCONTO}": `${depositPercent}%`,
    "{PERCENTUALE_SALDO}": `${balancePercent}%`,
  ...
  const updated = await recordSentQuote({id:quoteId,requestId:item.id,amountCents:quoteAmountCents,requestedPaymentCents,depositPercent,balancePercent,subject,body:deliveredBody,tokenHash:paymentTokenHash,actorEmail:user.email});
