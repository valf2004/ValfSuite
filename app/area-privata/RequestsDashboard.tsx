...
export type AvailabilityEvent = { id:string; requestId:string; eventType:"request_created"|"email_sent"|"payment_reported"|"payment_confirmed"|"status_changed"; fromStatus:string|null; toStatus:string|null; actorEmail:string|null; note:string|null; subject:string|null; body:string|null; amountCents:number|null; attachmentId:string|null; attachmentName:string|null; createdAt:string };
export type Status = "quote_requested" | "quote_sent" | "payment_reported" | "accepted" | "checked_in" | "police_registered" | "archived";
  ...
  return <>
    <section className="requests-panel"><div className="request-tabs" role="tablist" aria-label="Stato richiesta e prenotazione">{tabs.map(tab => <button key={tab.id} role="tab" aria-selected={active === tab.id} onClick={() => setActive(tab.id)}>{tab.label}<span>{requests.filter(item => item.status === tab.id).length}</span></button>)}</div>
      ...
      <p className="dashboard-notice" role="status">{notice}</p><div className="request-list" role="tabpanel">
        ...
        {visible.map(item => <RequestCard key={item.id} item={item} events={events.filter(event=>event.requestId===item.id)} active={active} busy={busy} today={today} quoteOpen={quoteFor===item.id} quoteDraft={quoteDraft} setQuoteDraft={setQuoteDraft} openQuote={openQuote} closeQuote={()=>setQuoteFor(null)} sendQuote={sendQuote} confirmationOpen={confirmationFor===item.id} confirmationDraft={confirmationDraft} setConfirmationDraft={setConfirmationDraft} openConfirmation={openConfirmation} closeConfirmation={()=>setConfirmationFor(null)} sendConfirmation={sendConfirmation} move={move}/>) }
      </div></section>
  </>;
}

function RequestCard({item,events,active,busy,today,quoteOpen,quoteDraft,setQuoteDraft,openQuote,closeQuote,sendQuote,confirmationOpen,confirmationDraft,setConfirmationDraft,openConfirmation,closeConfirmation,sendConfirmation,move}:{
  item:AvailabilityRequest; events:AvailabilityEvent[]; active:Status; busy:string|null; today:string; quoteOpen:boolean; quoteDraft:{subject:string;body:string;price:string};
  ...
  return <article className={`request-card priority-${priority.level}`}>
    ...
    <div className="request-actions">{item.status==="payment_reported"&&<button className="primary-action confirmation-action" disabled={busy===item.id} onClick={()=>openConfirmation(item)}>Conferma pagamento</button>}<button className={item.status==="quote_requested"?"primary-action":""} disabled={busy===item.id} onClick={()=>openQuote(item)}>{item.status==="quote_requested"?"Prepara preventivo":"Invia nuovo preventivo"}</button><a href={`mailto:${item.email}`}>Email libera</a><label className="status-control"><span>Stato</span><select value={statusValue} disabled={busy===item.id} onChange={event=>{if(event.target.value!==statusValue){setConflicts(undefined);setPendingStatus(event.target.value);}}}><option value="quote_requested">Richiesta preventivo</option><option value="quote_sent">Preventivo inviato</option><option value="payment_reported">Pagamento comunicato</option><option value="accepted">Accettata</option><option value="checked_in">Check-in eseguito</option><option value="police_registered">Questura registrata</option><option value="archived:completed">Archiviata · completata</option><option value="archived:cancelled">Archiviata · annullata</option><option value="archived:unavailable">Archiviata · non disponibile</option></select></label></div>
...
function eventTitle(event:AvailabilityEvent){if(event.eventType==="request_created")return "Richiesta ricevuta";if(event.eventType==="email_sent")return event.amountCents!=null?"Preventivo inviato":"Email inviata";if(event.eventType==="payment_reported")return "Pagamento comunicato dall’ospite";if(event.eventType==="payment_confirmed")return "Pagamento verificato · Conferma inviata";return `${event.fromStatus?statusLabel(event.fromStatus)+" → ":""}${statusLabel(event.toStatus||"")}`;}
