...
export function PrivateHeader({user,active}:{user:{email:string;name:string;picture?:string};active:"requests"|"calendar"|"database"|"settings"}) {
  return <header className="dashboard-header">
    ...
    <div className="dashboard-header-actions">
      <nav className="dashboard-nav" aria-label="Area riservata"><a className={active==="requests"?"active":""} aria-current={active==="requests"?"page":undefined} href="/area-riservata">Richieste</a><a className={active==="calendar"?"active":""} aria-current={active==="calendar"?"page":undefined} href="/area-riservata/calendario">Calendario</a><a className={active==="database"?"active":""} aria-current={active==="database"?"page":undefined} href="/area-riservata/database">Database</a></nav>
      <div className="private-account"><span><strong>{user.name}</strong><small>{user.email}</small></span><a className={`settings-link ${active==="settings"?"active":""}`} aria-label="Impostazioni" aria-current={active==="settings"?"page":undefined} title="Impostazioni" href="/area-riservata/impostazioni"><span aria-hidden="true">⚙</span></a><a href="/api/auth/logout">Esci</a></div>
    </div>
  </header>;
}
