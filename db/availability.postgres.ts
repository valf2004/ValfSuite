...
export async function recordCheckinSubmission(requestId:string,submission:CheckinSubmissionRecord,actorEmail?:string) {
  ...
  const rows=await sql.begin(async transaction=>{
    await transaction`INSERT INTO checkin_practices (request_id,state,language,guest_count,group_type,arrival_time,transport,tourism_type,arrival_notes,privacy_accepted_at,source,version,created_at,updated_at) VALUES (${requestId},'ready',${submission.language},${submission.guestCount},${submission.groupType},${submission.arrivalTime},${submission.transport},${submission.tourismType},${submission.arrivalNotes},${submission.privacyAcceptedAt},${actorEmail?"operator":"guest"},1,${createdAt},${createdAt}) ON CONFLICT (request_id) DO UPDATE SET state='ready',language=EXCLUDED.language,guest_count=EXCLUDED.guest_count,group_type=EXCLUDED.group_type,arrival_time=EXCLUDED.arrival_time,transport=EXCLUDED.transport,tourism_type=EXCLUDED.tourism_type,arrival_notes=EXCLUDED.arrival_notes,privacy_accepted_at=EXCLUDED.privacy_accepted_at,source=EXCLUDED.source,version=checkin_practices.version+1,last_error=NULL,updated_at=EXCLUDED.updated_at`;
    await transaction`DELETE FROM checkin_guests WHERE request_id=${requestId}`;
    for(const guest of submission.guests)await transaction`INSERT INTO checkin_guests (id,request_id,ordinal,alloggiati_type,first_name,last_name,birth_date,sex_code,citizenship_code,birth_country_code,birth_place_code,residence_country_code,residence_place_code,document_type_code,document_number,issue_place_code) VALUES (${`${requestId}:${guest.ordinal}`},${requestId},${guest.ordinal},${guest.alloggiatiType},${guest.firstName},${guest.lastName},${guest.birthDate},${guest.sexCode},${guest.citizenshipCode},${guest.birthCountryCode},${guest.birthPlaceCode},${guest.residenceCountryCode},${guest.residencePlaceCode},${guest.documentTypeCode},${guest.documentNumber},${guest.issuePlaceCode})`;
...
export async function getCheckinSubmission(requestId:string){
  ...
  const guests=await sql`SELECT * FROM checkin_guests WHERE request_id=${requestId} ORDER BY ordinal`;const values:Record<string,string>={"arrival-time":String(practice.arrival_time),transport:String(practice.transport),tourismType:String(practice.tourism_type||"NON SPECIFICATO"),"arrival-notes":String(practice.arrival_notes||"")};
  for(const guest of guests){const ordinal=Number(guest.ordinal);const prefix=ordinal===0?"lead":`guest-${ordinal}`;values[`${prefix}-name`]=String(guest.first_name);values[`${prefix}-surname`]=String(guest.last_name);values[`${prefix}-birth`]=dateValue(guest.birth_date);values[`${prefix}-sex`]=String(guest.sex_code);values[`${prefix}-citizenship`]=String(guest.citizenship_code);values[`${prefix}-birthCountry`]=String(guest.birth_country_code);if(guest.birth_place_code!=null)values[`${prefix}-birthPlace`]=String(guest.birth_place_code);if(guest.residence_country_code!=null)values[`${prefix}-residenceCountry`]=String(guest.residence_country_code);if(guest.residence_place_code!=null)values[`${prefix}-residencePlace`]=String(guest.residence_place_code);if(guest.document_type_code!=null)values[`${prefix}-documentType`]=String(guest.document_type_code);if(guest.document_number!=null)values[`${prefix}-documentNumber`]=String(guest.document_number);if(guest.issue_place_code!=null)values[`${prefix}-issuePlace`]=String(guest.issue_place_code);}
...
async function initializePostgres(client: Sql) {
  ...
  await client`CREATE TABLE IF NOT EXISTS checkin_practices (request_id text PRIMARY KEY REFERENCES availability_requests(id) ON DELETE CASCADE,state text NOT NULL DEFAULT 'draft',language text NOT NULL DEFAULT 'it',guest_count integer NOT NULL,group_type text NOT NULL,arrival_time text NOT NULL,transport text NOT NULL,tourism_type text NOT NULL DEFAULT 'NON SPECIFICATO',arrival_notes text NOT NULL DEFAULT '',privacy_accepted_at timestamptz NOT NULL,source text NOT NULL,version integer NOT NULL DEFAULT 1,last_error text,sent_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now())`;
  await client`ALTER TABLE checkin_practices ADD COLUMN IF NOT EXISTS tourism_type text NOT NULL DEFAULT 'NON SPECIFICATO'`;
  await client`CREATE TABLE IF NOT EXISTS checkin_guests (id text PRIMARY KEY,request_id text NOT NULL REFERENCES checkin_practices(request_id) ON DELETE CASCADE,ordinal integer NOT NULL,alloggiati_type text NOT NULL,first_name text NOT NULL,last_name text NOT NULL,birth_date date NOT NULL,sex_code text NOT NULL,citizenship_code text NOT NULL,birth_country_code text NOT NULL,birth_place_code text,residence_country_code text,residence_place_code text,document_type_code text,document_number text,issue_place_code text,UNIQUE(request_id,ordinal))`;
